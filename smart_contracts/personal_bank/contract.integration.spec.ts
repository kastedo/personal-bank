...
describe('personal bank contract', () => {
  ...
  test('deposit', async () => {
    ...
    const githubHandle = context.any.string()

    const result = await client.send.deposit({ args: { payTxn, githubHandle }, populateAppCallResources: true })
  ...
  test('deposit & withdraw', async () => {
    ...
    await client.send.deposit({ args: { payTxn, githubHandle }, populateAppCallResources: true })
